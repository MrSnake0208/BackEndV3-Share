...
关联契约：`~/YuanHub/docs/api-contract.md` 中“库存接口契约（交换协议 v2）”
...
具体状态码和所有权错误措辞以库存模块现有约定为准，至少覆盖：

| 场景 | HTTP | 建议错误码 |
|---|---:|---|
| 未登录或 JWT 失效 | 401 | 沿用全局认证错误 |
| 缺少或空 `account_id` | 400 | `ACCOUNT_ID_REQUIRED` |
| 当前用户无权访问子账号 | 403 或现有约定 | 沿用库存账号所有权错误 |
